...
if egfr < 60 or next_pred < 60:
    st.warning("🔶 腎功能低下：eGFR 小於 60，建議盡速就醫")
    st.markdown("""
    - **eGFR 是腎功能的重要指標**，當值低於 60 代表腎臟過濾功能下降。
    - 常見原因包括：高血壓、糖尿病、慢性腎炎。
    - 應定期檢驗腎功能、尿蛋白、控制血壓與血糖。
    - **建議至腎臟科進行詳細檢查與飲食指導。**
    """)
...
if "糖尿病" in chronic_illnesses:
    st.info("📌 糖尿病患者注意：長期高血糖會損傷腎臟、眼睛、神經")
    st.markdown("""
    - 建議控制糖化血色素 A1C ＜ 7%
    - 多攝取高纖食物、減少含糖飲料
    - 每年至少檢查一次腎功能與視網膜
...
if "高血壓" in chronic_illnesses:
    st.info("📌 高血壓控制建議：避免心血管疾病與腎臟傷害")
    st.markdown("""
    - 建議血壓控制目標 ＜ 130/80 mmHg
    - 減少鹽分攝取（每日 < 6g）、增加運動
    - 避免吸菸與過量飲酒
    - 高血壓若合併腎功能下降，風險更高
    """)
...
if smoking_freq > 5 or drinking_freq > 3 or chewing_freq > 3:
    st.info("🚭 抽菸、飲酒與嚼檳榔皆為高風險行為")
    st.markdown("""
    - **吸菸**：每增加一天抽菸，心血管疾病風險上升 10% 以上。
    - **嚼檳榔**：與口腔癌密切相關，尤其與吸菸聯合效果更強。
    - **飲酒**：過量飲酒會損傷肝腎功能，增加跌倒、中風風險。
    - 建議儘早尋求家庭醫學科協助進行衛教與戒斷。
    """)
    st.markdown("[➡ 家庭醫學科健康諮詢](https://www.cmuh.cmu.edu.tw/OnlineAppointment/DymSchedule?table=31300A&flag=first)")

# FRAIL 衰弱
if frail_score > 2:
    st.info("🏃‍♂️ 已達衰弱狀態，應主動介入改善")
    st.markdown("""
    - 建議進行營養補充（特別是蛋白質與維生素D）
    - 加入居家運動或社區健康促進課程
    - 檢查是否合併憂鬱或其他功能障礙
    - 衰弱為跌倒與住院的重要風險因子，務必重視
    """)
